  ...
  const handleDragEnd = useCallback(
    (result: DropResult) => {
      ...
      if (source.droppableId === destination.droppableId) {
        ...
        const updatedSource = sourceCards.map((c, i) => ({
          ...c,
          position: i,
        }));
    ...
    <div className="h-full flex flex-col board-bg">
        ...
        <div className="flex items-center gap-2">
          ...
          {(filters.assigneeId || filters.labelId || filters.overdue) && (
            <div className="flex items-center gap-1 flex-wrap">
              {filters.assigneeId && (
                <Badge variant="secondary" className="gap-1 text-xs pr-1">
                  {workspaceMembers.find(
                    (a) => a.user_id === filters.assigneeId,
                  )?.full_name ?? "Assignee"}
                  <button
                    onClick={() =>
                      setFilters((f) => ({ ...f, assigneeId: null }))
                    }
              ...
              {filters.labelId && (
                <Badge variant="secondary" className="gap-1 text-xs pr-1">
                  {allLabels.find((l) => l.label_id === filters.labelId)
                    ?.name ?? "Label"}
                  <button
                    onClick={() => setFilters((f) => ({ ...f, labelId: null }))}
                    className="ml-0.5 hover:text-destructive"
                  >
                    <X className="h-3 w-3" />
                  </button>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setFilters((f) => ({
                        ...f,
                        assigneeId:
                          f.assigneeId === a.user_id ? null : a.user_id,
                      }))
                    }
                ...
                {allLabels.map((l) => (
                  <button
                    ...
                    onClick={() =>
                      setFilters((f) => ({
                        ...f,
                        labelId: f.labelId === l.label_id ? null : l.label_id,
                      }))
                    }
      ...
      {(selectedCardId || externalCardId) && (
        <CardDetailModal
          ...
          onCardDeleted={handleCardDeleted}
          onBoardRefresh={() => {
            /* WebSocket handles refresh automatically */
          }}
        />
      )}
    </div>
