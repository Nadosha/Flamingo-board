...
export function CardDetailModal({ cardId, boardId, onClose, onCardDeleted, onBoardRefresh }: Props) {
  ...
  function handleAddAssignee(member: BoardMember) {
    ...
    startTransition(async () => {
      ...
      setCard((prev) => {
        ...
        return { ...prev, assignees: [...existing, { user_id: member.user_id, profile: member.profile }] };
      });
    });
  }
  ...
  function handleRemoveAssignee(userId: string) {
    ...
    startTransition(async () => {
      ...
      setCard((prev) => {
        if (!prev) return prev;
        return { ...prev, assignees: prev.assignees.filter((a) => a.user_id !== userId) };
      });
    });
  }
  ...
  function handleAddLabel(label: BoardLabel) {
    ...
    startTransition(async () => {
      ...
      setCard((prev) => {
        ...
        return { ...prev, labels: [...existing, { label_id: label.id, label: { name: label.name, color: label.color } }] };
      });
    });
  }
  ...
  function handleRemoveLabel(labelId: string) {
    ...
    startTransition(async () => {
      ...
      setCard((prev) => {
        if (!prev) return prev;
        return { ...prev, labels: prev.labels.filter((l) => l.label_id !== labelId) };
      });
    });
  }
                    ...
                    {boardMembers.filter((m) => !card.assignees.some((a) => a.user_id === m.user_id)).length > 0 && (
                      <Popover>
                        ...
                        <PopoverContent className="w-48 p-1" align="start">
                          {boardMembers
                            .filter((m) => !card.assignees.some((a) => a.user_id === m.user_id))
                            .map((member) => (
                              <button
                                key={member.user_id}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Popover onOpenChange={(open) => { if (!open) { setCreatingLabel(false); setNewLabelName(''); } }}>
                      ...
                      <PopoverContent className="w-52 p-2" align="start">
                        {boardLabels.filter((l) => !card.labels.some((cl) => cl.label_id === l.id)).map((label) => (
                          <button
                            key={label.id}
