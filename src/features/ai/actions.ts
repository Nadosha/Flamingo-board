...
export async function prioritizeBoardAction(boardId: string) {
  try {
    ...
    const message = err instanceof ApiError ? err.message : 'Failed to prioritize board';
...
export async function decomposeCardAction(
  ...
  try {
    ...
    const message = err instanceof ApiError ? err.message : 'Failed to decompose card';
    return { error: message };
  }
}
...
export async function generateStandupAction(boardId: string) {
  try {
    ...
    const message = err instanceof ApiError ? err.message : 'Failed to generate standup';
    return { error: message };
  }
}
